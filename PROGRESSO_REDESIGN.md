...
## Próximo checkpoint
- Com as Fases 3 e 4 concluídas e a Fase 6 avançada (Home/busca unificadas), retomar Fase 8 (responsividade) ou Fase 10 (acessibilidade no resto do app, fora do editor de posição), conforme o usuário priorizar na próxima sessão.
...
- Pré-existente, não é regressão desta sessão: clicar em "Analisar" ou no logo com uma partida já carregada não volta pra tela de busca (só reseta os outros modos) — o app não tem um jeito de "descarregar" a partida atual. Vale avaliar se isso incomoda, mas está fora do que foi pedido aqui.
