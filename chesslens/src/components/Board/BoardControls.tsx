...
export function BoardControls({
  ...
  return (
    ...
    >
      <button className={NAV_BTN} style={{ color: 'var(--text)', backgroundColor: 'var(--surface2)' }} onClick={onFlip} title="Inverter tabuleiro">
        <FlipBoardIcon width={15} height={15} />
      </button>

      <div style={{ width: 1, height: 22, background: 'var(--border)', margin: '0 2px', flexShrink: 0 }} />

      <button className={NAV_BTN} style={{ color: 'var(--text)', backgroundColor: 'var(--surface2)' }} onClick={withPauseAutoplay(onFirst)} disabled={atStart || !isLoaded} title="Início">
        <FirstMoveIcon width={15} height={15} />
      </button>
      <button className={NAV_BTN} style={{ color: 'var(--text)', backgroundColor: 'var(--surface2)' }} onClick={withPauseAutoplay(onPrev)} disabled={atStart || !isLoaded} title="Anterior">
        <PrevMoveIcon width={15} height={15} />
      </button>
      <button
        className={NAV_BTN}
        style={{ color: 'var(--on-accent, #fff)', backgroundColor: 'var(--accent)' }}
      ...
      <button className={NAV_BTN} style={{ color: 'var(--text)', backgroundColor: 'var(--surface2)' }} onClick={withPauseAutoplay(onNext)} disabled={atEnd || !isLoaded} title="Próximo">
        <NextMoveIcon width={15} height={15} />
      </button>
      <button className={NAV_BTN} style={{ color: 'var(--text)', backgroundColor: 'var(--surface2)' }} onClick={withPauseAutoplay(onLast)} disabled={atEnd || !isLoaded} title="Último">
        <LastMoveIcon width={15} height={15} />
      </button>

      <span className="cl-display" style={{ fontSize: 11.5, color: 'var(--text-muted)', fontWeight: 700, minWidth: 50, textAlign: 'center', flexShrink: 0 }}>
