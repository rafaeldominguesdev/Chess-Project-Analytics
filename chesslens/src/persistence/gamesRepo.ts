...
export async function saveGame(url: string, pgn: string, gameInfo: GameInfo): Promise<void> {
  await putItem<StoredGame>(STORES.games, { url, pgn, gameInfo, savedAt: Date.now() })
}
